...
from models import Booking, Route, RouteStop, Trip, Vehicle
    ...
    def patch(self, vehicle_id):
        ...
        if "number_plate" in data:
            if not isinstance(data["number_plate"], str):
                return {
                    "error": "number_plate must be a string"
                }, 400
...
class ProviderRouteTripsResource(Resource):
    ...
    def post(self, route_id):
        ...
        destination_stop = db.session.get(
            RouteStop,
            destination_id
        )
        vehicle = db.session.get(Vehicle, vehicle_id)
...
class BookingStatisticsResource(Resource):
    ...
    def get(self):
        ...
        if group_by != "day":
            return {
                "error": (
                    "Only group_by=day is currently supported"
                )
            }, 400
...
api.add_resource(
    ProviderBookingsResource,
    "/api/v1/provider/bookings"
)

api.add_resource(
    BookingStatisticsResource,
    "/api/v1/provider/booking-statistics"
)

api.add_resource(
    TripBookingsResource,
